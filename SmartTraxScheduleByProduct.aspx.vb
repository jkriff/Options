...
Partial Class SmartTraxScheduleByProduct
    ...
    Private Sub BindStaticData()
        ...
        Try
            ...
            If dr.Read Then
                ...
                For I As Integer = 1 To CInt(startHour)
                    dgSchedules.Columns(I).Visible = False
                Next

                For I As Integer = CInt(endHour) + 2 To 24
                    dgSchedules.Columns(I).Visible = False
                Next
            End If
            ...
            dtWeeks.Columns.Add("date")
            dtWeeks.Columns.Add("description")

            row = dtWeeks.NewRow
            row("date") = CDate("1/1/1900")
            row("description") = "[select week]"
            dtWeeks.Rows.Add(row)

            For I As Integer = 1 To 12
                row = dtWeeks.NewRow
                row("date") = curDate
                row("description") = "Week of " & curDate.ToString("MM/dd/yyyy")
                dtWeeks.Rows.Add(row)
                curDate = curDate.AddDays(7)
            Next

            ddlWeeks.DataSource = dtWeeks
            ddlWeeks.DataValueField = "date"
            ddlWeeks.DataTextField = "description"
            ddlWeeks.DataBind()
    ...
    Private Sub BindMasterData()
        If ddlProducts.SelectedIndex > 0 And ddlWeeks.SelectedIndex > 0 Then
            Dim db As New SqlConnection(GetConnectString())

            Try
                Dim dtSchedules As New DataTable
                dtSchedules.Columns.Add("Date")
                ...
                Dim curDate As Date = ddlWeeks.SelectedValue
                For I As Integer = 1 To 7
                    ...
                    sql &= "WHERE       [store] = " & _store & " "
                    sql &= "        AND [productID] = " & ddlProducts.SelectedValue & " "
                    sql &= "        AND [yyyymmdd] = '" & curDate.ToString("yyyyMMdd") & "' "
                    ...
                    row("Date") = curDate.ToString("MM/dd/yyyy")
                    row("Hour00") = 0
                    row("Hour01") = 0
                    row("Hour02") = 0
                    row("Hour03") = 0
    ...
    Private Sub dgSchedules_EditSchedule(ByVal arg As String)
        Dim curDate As Date = CDate(arg.Substring(0, arg.IndexOf("|")))
        ...
        Try
            ...
            hideEditDate.Value = curDate.ToString("MM/dd/yyyy")
            hideEditHour.Value = hour
            litModalTitle.Text = "Edit " & hideEditDate.Value & ", hour " & hour
            If dr.Read() Then
    ...
    Protected Sub btnEditScheduleOK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEditScheduleOK.Click
        ...
        Try
            ...
            sql &= "WHERE       [store] = " & _store & " "
            sql &= "        AND [productID] = " & ddlProducts.SelectedValue & " "
            sql &= "        AND [yyyymmdd] = '" & CDate(hideEditDate.Value).ToString("yyyyMMdd") & "' "
            ...
            Else
                ...
                sql &= "            1, "
                sql &= "            " & _store & ", "
                sql &= "            " & ddlProducts.SelectedValue & ", "
                sql &= "            '" & CDate(hideEditDate.Value).ToString("yyyyMMdd") & "', "
    ...
    Private Sub DoSuggestion(ByVal sYYYYMMDD As String, ByVal weekday As Integer, ByVal percentage As Integer)
        Dim db As New SqlConnection(GetConnectString())
        ...
        Try
            ...
            Dim multiplier As Decimal = Convert.ToDecimal(percentage) / 100
    ...
    Protected Sub dgSchedules_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgSchedules.ItemCommand
        ...
        Try
            Dim sYYYYMMDD As String = CDate(e.Item.Cells(0).Text).ToString("yyyyMMdd")
            Dim weekday As Integer = CDate(e.Item.Cells(0).Text).DayOfWeek + 1
            ...
            If e.CommandName.ToUpper = "SUGGEST" Then
                hidePercentageDate.Value = sYYYYMMDD
                hidePercentageWeekday.Value = weekday
            ...
            ElseIf e.CommandName.ToUpper = "DIALUP" Then
                ...
                DoSuggestion(sYYYYMMDD, weekday, percentage)
    ...
    Protected Sub dgSchedules_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgSchedules.ItemCreated
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            Dim btnClear As Button = CType(e.Item.Cells(29).Controls(0), Button)
    ...
    Protected Sub btnPercentageOK_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPercentageOK.Click
        DoSuggestion(hidePercentageDate.Value, hidePercentageWeekday.Value, ddlPercentage.SelectedValue)
    End Sub
End Class
