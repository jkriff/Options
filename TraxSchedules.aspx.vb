...
#End Region
    ...
    Private Sub BindProductData()
        ...
        Try
            ...
            sql &= "            CASE WHEN p2.[Desc] IS NULL THEN "
            sql &= "                CONVERT(varchar(10),productID) + ' - ' + LTRIM(RTRIM(p1.[Desc])) "
            sql &= "            ELSE "
            sql &= "                CONVERT(varchar(10),productID) + ' - ' + LTRIM(RTRIM(p1.[Desc])) + ' (upgrades ' + LTRIM(RTRIM(p2.[Desc])) + ')' "
            sql &= "            END as 'Desc' "
            sql &= "FROM        TraxProducts tp "
            sql &= "JOIN        PLUs p1 "
            sql &= "        ON  p1.client = tp.client "
            sql &= "        AND p1.store = tp.store "
            sql &= "        AND p1.plu = tp.plu "
            sql &= "LEFT JOIN   PLUs p2 "
            sql &= "        ON  p2.client = tp.client "
            sql &= "        AND p2.store = tp.store "
            sql &= "        AND p2.plu = tp.upgradePLU "
            sql &= "WHERE       tp.client = 1 "
            sql &= "        AND tp.store = " & _store & " "
            sql &= "        AND tp.dtDeleted IS NULL "
            ...
            ddlProducts.DataTextField = "Desc"
    ...
    Private Sub BindMasterData()
        ...
        Try
            ...
            sql &= "            ts.productID as 'ProductID', "
            sql &= "            p1.[desc] as 'ProductDesc', "
            sql &= "            p2.[desc] as 'UpgradeDesc', "
            sql &= "            ts.numberOfProducts as 'NumberOfProducts', "
            ...
            sql &= "JOIN        TraxProducts tp "
            sql &= "        ON  tp.client = ts.client "
            sql &= "        AND tp.store = ts.store "
            sql &= "        AND tp.productID = ts.productID "
            sql &= "JOIN        PLUs p1 "
            sql &= "        ON  p1.client = tp.client "
            sql &= "        AND p1.store = tp.store "
            sql &= "        AND p1.plu = tp.plu "
            ...
            For Each row As DataRow In dt.Rows
                ...
                product = row("ProductID").ToString.Trim & " - " & row("ProductDesc").ToString.Trim
                If Not IsDBNull(row("UpgradeDesc")) Then
                    product &= " (upgrades " & row("UpgradeDesc").ToString.Trim & ")"
                End If
                row("Product") = product
            Next
    ...
    Private Sub dgSchedules_ItemDataBound(ByVal sender As Object, ByVal e As DataGridItemEventArgs) Handles dgSchedules.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            e.Item.Attributes.Add("onclick", "loadItems(this,'" & e.Item.DataItem("ScheduleID") & "','" & e.Item.DataItem("SchedulerID") & "','" & e.Item.DataItem("TypeID") & "','" & e.Item.DataItem("DateParameter") & "','" & e.Item.DataItem("StartTime") & "','" & e.Item.DataItem("EndTime") & "','" & e.Item.DataItem("ProductID") & "','" & e.Item.DataItem("GenerateEvery") & "');")
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        ...
        Dim productID As String = Request.Form("ddlProducts")
        ...
        Try
            ...
            Dim numberOfProducts As Integer = DateDiff(DateInterval.Minute, startTimeDate, endTimeDate) / generateEvery
            ...
            sql = "INSERT TraxSchedules (client, store, scheduleID, schedulerID, typeID, dateParameter, startTime, endTime, productID, generateEvery, numberOfProducts, message, dtCreated, dtUpdated) " & _
                  "VALUES (1, " & _store & ", " & newScheduleID & ", " & schedulerID & ", " & typeID & ", '" & dateParameter & "', '" & startTime & "', '" & endTime & "', " & productID & ", " & generateEvery & ", " & numberOfProducts & ", '', GETDATE(), GETDATE())"
            cmd = New SqlCommand(sql, db)
            cmd.ExecuteNonQuery()

        Catch ex As SqlException
            lblErrors.Text = "Error Adding New Schedule<br/><br/>" & ex.ToString
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Catch ex As SqlException
            lblErrors.Text = "Error Updating Schedule<br/><br/>" & ex.ToString
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        Catch ex As Exception
            lblErrors.Text = "Error Deleting Schedule<br/><br/>" & ex.ToString
